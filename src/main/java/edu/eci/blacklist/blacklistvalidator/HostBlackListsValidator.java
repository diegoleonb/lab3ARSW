...
public class HostBlackListsValidator {
     ...
     */
     public List<Integer> checkHost(String ipaddress,int cores){
        ...
        HostBlacklistsDataSourceFacade skds = HostBlacklistsDataSourceFacade.getInstance();
        int groups = skds.getRegisteredServersCount()/cores;
        for(int i = 0; i< cores;i++){
            if(i<cores-1){
                blackThread.add(new Threadblacklistvalidator(ipaddress,(i*groups),((i+1)*groups)-1,skds));
            }
            else if(i == cores-1 ){
                 blackThread.add(new Threadblacklistvalidator(ipaddress,(i*groups),skds.getRegisteredServersCount(),skds));
            }
